import RefExampleComponentMeta from 'D:\\babel-test\\temp\\lowcode\\ref-example-component\\meta'
import RefInputComponentMeta from 'D:\\babel-test\\temp\\lowcode\\ref-input-component\\meta'
...
function fillRealVersion(meta, packageName = 'multi-component', version = '0.1.0', basicLibraryVersion={"@alifd/next":"1.25.23","@alifd/meet":"2.6.3","antd":"4.17.3"}) {
  if (!meta || !version) {
    return meta;
  }
  const { npm } = meta;
...
const componentMetas = [RefExampleComponentMeta,RefInputComponentMeta];
const components = [];
const npmInfo = {};
...
if (!execCompile) {
  window.MultiComponentMeta = { components, componentList };
}